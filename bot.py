...
BOT_TOKEN = os.environ.get("BOT_TOKEN",
                           "8128964123:AAHf0vLSgUmmN_3QIzzI6Gqem8-Zg9sTmQE")
...
async def start_command(update: Update,
    ...
    if chat.type not in [Chat.GROUP, Chat.SUPERGROUP]:
        await message.reply_text(
            "This bot is designed for group chats. Please add me to a group!")
        return
    ...
    if not is_admin:
        await message.reply_text(
            "Only group administrators can activate the translation bot.")
        return
...
async def language_command(update: Update,
                           context: ContextTypes.DEFAULT_TYPE) -> None:
    """
    ...
    if language_utils.is_valid_language_code(language_code):
        ...
        await update.message.reply_text(
            f"Your language has been set to {language_name} ({language_code})."
        )
    else:
        await update.message.reply_text(
...
async def handle_message(update: Update,
    ...
    for recipient_id, recipient_lang in user_languages.items():
        ...
        try:
            translated_text = language_utils.translate_text(
                text, source_lang=detected_lang, target_lang=recipient_lang)
            ...
            if translated_text and translated_text.lower() != text.lower():
                ...
                logger.debug(
                    f"Sent translation from {detected_lang} to {recipient_lang} for user {recipient_id}"
                )
        except Exception as e:
